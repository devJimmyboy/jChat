...
Chat = {
  ...
  loadEmotes: function (channelID) {
    ...
    ;['emotes/global', 'users/twitch/' + encodeURIComponent(channelID)].forEach((endpoint) => {
      $.getJSON('https://api.betterttv.net/3/cached/' + endpoint).done(function (res) {
        if (Array.isArray(res.bots))
          res.bots.forEach((bot) => {
            if (!Chat.info.bots.includes(bot)) Chat.info.bots.push(bot)
          })
      ...
      if (!Chat.info.hideBadges) {
        ...
        $.getJSON('https://api.7tv.app/v2/badges?user_identifier=login')
          .done(function (res) {
            ...
            Chat.info.seventvCosmetics = res.paints
          })
          .fail(function () {
  ...
  loadUserBadges: function (nick, userId, resolve, reject) {
    Chat.info.userBadges[nick] = []
    Chat.info.userPaints[nick] = []
              ...
              css: {
                background:
                  paint.function === 'url'
                    ...
                    : `${paint.repeat ? 'repeating-' : ''}${paint.function}(${paint.function.startsWith('linear') ? `${paint.angle}deg` : paint.shape},${paint.stops
  ...
  write: async function (nick, info, message) {
    if (info) {
      ...
      if (Chat.info.hideBadges) {
        ...
        if (Chat.info.userBadges[nick]) {
          if (Chat.info.loadingBadges[nick]) await Chat.info.loadingBadges[nick]
        ...
        for (cheerType of Object.entries(Chat.info.cheers)) {
          ...
          if (message.search(regex) > -1) {
            ...
            if (!parsed) {
              ...
              message =
                '<img class="cheer_emote" src="' + cheerType[1][closest].image + '" /><span class="cheer_bits" style="color: ' + cheerType[1][closest].color + ';">' + bits + '</span> ' + message
              parsed = true
            }
          }
        }
        ...
        data.data.split('\r\n').forEach((line) => {
          ...
          switch (message.command) {
            ...
            case 'PRIVMSG':
              ...
              if (!Chat.info.hideBadges) {
                if (Chat.info.bttvBadges && Chat.info.seventvBadges && Chat.info.chatterinoBadges && Chat.info.ffzapBadges && (!Chat.info.userBadges[nick] || !Chat.info.userPaints[nick]))
                  Chat.info.loadingBadges[nick] = new Promise((resolve, reject) => Chat.loadUserBadges(nick, message.tags['user-id'], resolve, reject))
              }
